Function Set-PVSafe {
	...
	param(
		...
		[Parameter(Mandatory = $False, ValueFromPipelineByPropertyName = $False)][int]$requestRetention,
		[Parameter(Mandatory = $False, ValueFromPipelineByPropertyName = $False)][ValidateSet("None", "TextOnlyFilter")][string]$safeFilter,
		...
		[Parameter(Mandatory = $False, ValueFromPipelineByPropertyName = $False)][switch]$alwaysNeedConfirmation,
