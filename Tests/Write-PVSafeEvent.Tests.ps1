	...
	InModuleScope $ModuleName {
		...
		Context "Default" {
			...
			It "executes without exception" {
				...
				{$InputObj | Write-PVSafeEvent -sourceID x -eventTypeID x -data x} | Should Not throw

			}

			It "invokes expected pacli command" {

				$InputObj | Write-PVSafeEvent -sourceID x -eventTypeID x -data x

				Assert-MockCalled Invoke-PACLICommand -Times 1 -Exactly -Scope It -ParameterFilter {

					$PacliCommand -eq "ADDEVENT"
